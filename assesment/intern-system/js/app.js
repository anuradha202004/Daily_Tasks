const App = {
    ...
    reassignTask(taskId) {
        ...
        // Get matching interns (ACTIVE with required skills)
        const matchingInterns = AppState.interns.filter(intern => {
            return intern.status === 'ACTIVE' && 
                  task.requiredSkills.every(skill => intern.skills.includes(skill));
        });

        if (matchingInterns.length === 0) {
            Renderer.showAlert('No active interns match this task\'s required skills', 'error');
            return;
        }

        // Store task ID for reassignment modal
        AppState.reassignmentData = { taskId, matchingInterns };
        
        // Show reassignment modal
        Renderer.showReassignmentModal(task, matchingInterns);
    },

    confirmReassignment(newInternId) {
        const { taskId } = AppState.reassignmentData;
        const task = AppState.tasks.find(t => t.id === taskId);
        const oldIntern = AppState.interns.find(i => i.id === task.assignedTo);
        const newIntern = AppState.interns.find(i => i.id === newInternId);

        if (!task || !newIntern) return;

        const oldAssignment = oldIntern ? oldIntern.name : 'Unassigned';
        task.assignedTo = newInternId;
        StorageManager.saveToStorage();

        this.addLog('TASK_REASSIGNED', 
            `Task "${task.title}" reassigned from ${oldAssignment} to ${newIntern.name}`);
        
        Renderer.hideReassignmentModal();
        Renderer.showAlert(`Task reassigned to ${newIntern.name}!`);
        Renderer.renderTasks();
        Renderer.renderInterns();
        Renderer.renderDashboard();
    },
